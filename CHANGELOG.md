...
- **v0.3 priority #5 — community-skill head-to-head (meta-paap v2 vs 4 community-authored skills)** ([`05-EVALUATION/community-head-to-head.md`](./05-EVALUATION/community-head-to-head.md)). Repeats priority #3's protocol with **community-authored** handwritten arms instead of the rubric author's local production skills. 4 archetype-stratified skills (`obra/test-driven-development`, `anthropics/canvas-design`, `chrisvoncsefalvay/d3-viz`, `gstack/plan-ceo-review`) × 3 inputs × 3 cross-vendor judges = 36 paired quality judgments. **Aggregate: v2-generated wins 29/36 (81%)** with all 3 vendors agreeing (Claude 11/12, GPT-5.4 9/12, Gemini-3 9/12). Per-workflow: canvas-design 9/9 unanimous v2; gstack/plan-ceo-review 9/9 unanimous v2 (despite gstack's ~600 lines of templated PREAMBLE — the deferred-candidate #21 SKILL.md-as-build-artifact pattern); chrisvoncsefalvay/d3-viz 7/9 v2; **obra/TDD 5/9 community** (the only workflow where production-iterated discipline depth holds the line). Inter-judge agreement: 9/12 unanimous (much higher than priority #3's 7/15). **0/36 provenance accuracy** across all judges — every blind judge predicted provenance opposite to reality on every pair (judges associate visible structure with hand-tuning; v2 produces more visible structure → systematic mis-attribution). The framework can now defensibly claim v2-generated meets or beats community-authored quality on workflows where quality is articulable from a brief (Creative voice, Procedural mode-declarations, Reference completeness); production iteration holds the line where quality lives in specifics no brief captures (TDD discipline depth). **Honest caveat-saturation:** the rubric author also designed v2's prompt — measurement-side self-bias is real; v0.4 priorities include third-party rubric author + third-party hand-authored skills (random sample from N=80 corpus) + human-rater layer.
- `05-EVALUATION/community-head-to-head-raw-data.md` — verbatim 24 outputs + 36 judge JSONs from the priority #5 experiment.

- **v0.3 priority #4 — meta-paap v2.0 (rationalizations + self-observation).** Upgrades the skill generator to address two empirically-grounded gaps. **(a) Persona depth:** discipline-enforcing skills now produce an Iron Law + Rationalizations to Reject table (the pattern that made `obra/test-driven-development` distinctive). v0.3 priority #3 found generated skills systematically lacked this depth. **(b) Self-observation loop:** meta-paap reads `~/.claude/meta-paap/learnings.jsonl` in Phase 1.5b and appends to it after self-critique. Closes the framework's own honest C grade on principle #24, externally validated by GPT-5.4 (7/7 near-F) and Gemini-3 (5/6 F-or-near-F). **(c) v0.3 principle checks:** Phase 4 self-critique now explicitly checks #23 (host-portable), #24 (self-observation), #25 (spawn-detection) — 11 critical checks total (was 7 in v1). Backwards-compatible — v1 skills run unchanged; v2 generates higher-depth output for new skills.
- **Validation experiment (N=1, directional):** regenerated humanizer with v2 meta-paap, ran it on the same technical AI passage from priority #3, blind-judged against the v0.3-handwritten output. **v2-generated won 21/25 vs handwritten 11/25** — 10-point margin, judge correctly identified provenance both ways. The handwritten arm got task-compliance capped on 3 dimensions (substance preservation, AI-signature removal, editorial discipline) because it changed the argument — exactly the failure mode the rationalizations-to-reject pattern + the rubric's meaning-preservation note were designed to catch. **Caveat:** priority #3 had handwritten 8/9 humanizer wins; this validation is N=1 directional confirmation, not full replication. v0.4 work: full priority #3 re-run with v2-generated arm.
- **Reflexive self-audit upgraded:** principle #24 grade re-evaluated from C to **B+** based on the now-implemented closed write+read loop in meta-paap. Aggregate goes from A− back to **A**. The grade is grounded in observable mechanism (auditable in `06-META-PAAP/SKILL.md` Phase 1.5b + Output sections), not just claim. Why not A: the read-side is intentional simple substring match (not RAG), and `/paap-eval` doesn't yet consume prior calibration data — both deferred to v0.4.
- `06-META-PAAP/references/architecture-checklist.md` extended with §21 (Persona Depth — Iron Law + Rationalizations) and §22 (Self-Observation — write+read learnings loop) so generated skills can copy the patterns without meta-paap inlining them.
